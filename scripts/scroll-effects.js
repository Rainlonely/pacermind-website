    ...
    const TRANSLATIONS = {
        ...
        privacy: {
            en: {
                ...
                "privacy.subtitle": "PacerMind is a local-first running app. Your running history stays on your device, and we do not operate a server that stores your personal workout data.",
                ...
                "privacy.section4.title": "4. AI Features",
                "privacy.section4.text": "Some PacerMind features use third-party large language model providers to generate training feedback, summaries, or insights. Currently, AI services in mainland China are provided through Qwen, and AI services outside mainland China are provided through OpenAI GPT models. When you use an AI feature, the necessary workout context, notes, or prompt content may be sent to the relevant provider solely to generate the requested response.",
                "privacy.section4.extra": "We may change model providers, model versions, or routing logic from time to time, including for quality, reliability, availability, compliance, or cost reasons. These changes may occur without prior notice, but our purpose remains to provide the best practical feedback experience for users.",
                ...
                "privacy.section6.text": "Your PacerMind data is stored locally in the app container on your device. Deleting the app deletes the local app data managed by iOS. We do not keep separate server-side backups of your workout history, notes, plans, route data, or AI results.",
            ...
            zh: {
                ...
                "privacy.subtitle": "PacerMind 是一款本地优先的跑步应用。你的跑步历史保存在你的设备上，我们不会运营用于接收或存储你个人训练数据的服务器。",
                ...
                "privacy.section4.title": "4. AI 功能",
                "privacy.section4.text": "PacerMind 的部分功能会使用第三方大语言模型服务来生成训练反馈、总结或洞察。目前，中国大陆地区的 AI 服务由 Qwen 提供，中国大陆以外地区的 AI 服务由 OpenAI GPT 模型提供。当你使用 AI 功能时，为生成你请求的结果，必要的训练上下文、笔记或提示内容可能会发送给相应模型服务商。",
                "privacy.section4.extra": "我们可能会根据反馈质量、稳定性、可用性、合规要求或成本等因素，不时调整模型服务商、模型版本或调用路由。这类调整可能不会提前通知用户，但目的始终是为用户提供尽可能好的反馈体验。",
                ...
                "privacy.section6.text": "你的 PacerMind 数据保存在设备本地的应用容器中。删除应用会删除由 iOS 管理的本地应用数据。我们不会在服务器端另行备份你的训练历史、笔记、计划、路线数据或 AI 结果。",
