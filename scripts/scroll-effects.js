    ...
    const TRANSLATIONS = {
        ...
        privacy: {
            ...
            zh: {
                ...
                "privacy.section5.text": "只有在你明确同意后，PacerMind 才会使用位于美国的 PostHog Cloud 来了解功能采用情况并改进产品。我们会发送基础应用生命周期事件和一组受限的产品里程碑，例如完成引导、连接训练来源、完成首次导入、打开训练或计划详情、生成洞察或计划，以及完成分享。事件属性仅限于来源、洞察类型、计划场景和分享格式等粗粒度类别。PostHog 还可能处理随机生成的匿名标识符，以及应用版本、设备型号、操作系统版本、语言区域、时区和处理网络请求时必然接收的网络信息等技术上下文。PacerMind 会在每个统计事件中明确关闭 GeoIP 位置富化。",
